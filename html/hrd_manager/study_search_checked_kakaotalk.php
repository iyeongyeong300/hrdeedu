...
if($mode=="Auth") {
	...
	//foreach ====================================================================================================================================================
		...
		$Sql = "SELECT a.ServiceType, AES_DECRYPT(UNHEX(b.Mobile),'$DB_Enc_Key') AS Mobile, b.Name, DATEDIFF(NOW(),a.LectureStart) AS diff_day, c.CyberEnabled, c.CyberURL FROM 
			Study AS a 
			LEFT OUTER JOIN Member AS b ON a.ID=b.ID 
			LEFT OUTER JOIN Company AS c ON a.CompanyCode=c.CompanyCode 
			WHERE a.Seq=$seq";
