...
function getSaveData(target){
	...
	return getCookie(target);
}

function setSaveData(target, value){
	saveData = setCookie(target,value,1);
...
// 수정자 : 오투잡개발자 
// 20210117 수정완료
// isMenuVisible 를 flag 로 쓰는방법은 잘못됨.
// 호출할때마다 새로 false 를 해주기 때문에 올바르게 작동하지 않음
// 팝업되는 창을 기준으로 보여지고 있는가 보여지고 있지않은가를 기준으로 작동되게끔 수정완료

$('._menuBarBtn').click(function(){
    if($('._menuBarPup').css('display') == 'none'){
...
$('._contentsPlayAndStopBtn').click(function(){
    ...
    response(valume);
	$('#_nextPlay').hide();
	$('#_downloadPdfBtn').hide();
	$('#_downloadHwpBtn').hide();
