...
$SiteName = "HRD에셋";
$CertSiteName = "HRD에셋";
$UPLOAD_DIR = "/home/LMS/public_html/upload";


$HomeDirectory = "/home/LMS/public_html";
...
$CheckPlus_sitecode = "BU359";
$CheckPlus_sitepasswd = "vHtoTTutF2sG";

$IPIN_CheckPlus_sitecode = "FS66";
$IPIN_CheckPlus_sitepasswd = "hrd^^190302190302";
...
for($i=0; $i<sizeof($MobileArray); $i++){ 
	if(preg_match("/$MobileArray[$i]/", strtolower($HTTP_USER_AGENT))){ $MobilecheckCount++; break; } 
}

if($MobilecheckCount >= 1) { //모바일인 경우
	$UserDevice = "Mobile";
}else{ //PC인 경우
	$UserDevice = "PC";
}
...
$Menu02ParentCategory = 2;
$Menu03ParentCategory = 3;
...
$kakaotalk_array = array(
	"mtm" => "T20190131-6|[EK티쳐] 1:1문의 답변이 등록되었습니다. 확인 부탁드립니다. (나의강의실-상담신청내역)", //1:1 문의 답변 등록 알림

	"cronAuth" => "T20190131-4|#{이름} 수강생님! 안녕하세요? EK티쳐입니다.
...
#{인증URL}", //오전9시 10분 cron 자동발송 - 개강 다음날에 발송
	...
	"cronStart1" => "T20190131-1|안녕하세요. EK티쳐입니다. 신청하신 교육에 대한 개강 안내를 드립니다.
...
(본 메세지는 #{소속업체명} 사업주께서 저희 교육기관에 인터넷교육을 신청하여 안내드리는 내용입니다.)", //오전9시 10분 cron 자동발송- 비환급, 개강 당일
	...
	"cronStart2" => "T20190131-3|안녕하세요. EK티쳐입니다. 신청하신 교육에 대한 개강 안내를 드립니다.
[#{회사명}] #{소속업체명} 인터넷교육이 #{시작일} 시작되었습니다!
접속 정보 : 학습사이트:#{도메인} 아이디:#{아이디} 초기비밀번호:1111 
...
function kakaotalk_send($msg_type,$msg_mobile,$msg_var,$send_date) {
	...
	if($msg_mobile) {
		...
		$TRAN_PHONE = "01032465683"; //수신번호
		$TRAN_SUBJECT = "EK티쳐입니다"; //MMS전환시 문자 제목
		$TRAN_REPLACE_TYPE = "L"; //전환전송 타입 'S', 'L', 'N'  'S' : SMS 전환전송 'L' : LMS 전환전송 'N' : 전환전송 하지않음

		$query = "INSERT INTO HRDTALK.MTS_ATALK_MSG(TRAN_SENDER_KEY,TRAN_TMPL_CD,TRAN_CALLBACK,TRAN_PHONE,
...
function mts_mms_send($phone,$msg,$callback,$etc1) {
	...
	if($phone) {
		$data=array(
			"auth_code"=>"fe55wMSf/O7z7KVfUeb81w==",
			//"sender_key"=>"bc54308732b7789e673004d47b4e6a5ce75110b1",
			"sender_key"=>"bc54308732b7789e673004d47b4e6a5ce75110b1",
		...
		$query = "INSERT INTO HRDTALK.MTS_MMS_MSG(TRAN_PHONE,TRAN_CALLBACK,TRAN_MSG,TRAN_DATE, TRAN_TYPE, TRAN_ETC1, TRAN_REPORTDATE, TRAN_RSLTDATE, TRAN_RSLT) VALUES ('$phone','$callback','$msg',now(), 4,'$etc1', '$report_date', '$result_date', '$result_code')";
		$result = mysqli_query($connect, $query);
...
function StudyProgressStatus($ServiceType,$LectureCode,$Study_Seq) {
	...
	return $Status_msg01."|".$Status_msg02."|".$Status_msg03;
	//return $ChapterCount."|";
