...
<table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
	...
	<tr>
		...
		?>
		<input type="button" name="Btn" id="Btn" value="금액 저장" class="btn_inputSm01" style="width:80px" onclick="PaymentSave(<?=$k?>,'<?=$LectureStart?>','<?=$LectureEnd?>','<?=$CompanyCode?>')" /><br>
		<input type="button" name="Btn" id="Btn" value="결제 요청" class="btn_inputSm01" style="width:80px" onclick="PayStatusSave(<?=$k?>,'<?=$LectureStart?>','<?=$LectureEnd?>','<?=$CompanyCode?>')" /><br>
		<input type="button" name="Btn" id="Btn" value="결제 완료" class="btn_inputSm01" style="width:80px" onclick="PayStatusComplete(<?=$k?>,'<?=$LectureStart?>','<?=$LectureEnd?>','<?=$CompanyCode?>')" />
		<?
		}
		?>
		<?
		...
		<input type="button" name="Btn" id="Btn" value="결제 요청 취소" class="btn_inputSm01" style="width:100px" onclick="PayStatusCancelSave(<?=$k?>,'<?=$LectureStart?>','<?=$LectureEnd?>','<?=$CompanyCode?>')" />
		<?
		}
		?>
		<?
		...
		if($PayStatus=='Y') { //결제완료시
		?>
		<input type="button" name="Btn" id="Btn" value="결제 취소(환불)" class="btn_inputSm01" style="width:100px" onclick="PaymentCancelSave(<?=$k?>,'<?=$LectureStart?>','<?=$LectureEnd?>','<?=$CompanyCode?>')" />&nbsp;&nbsp;&nbsp;&nbsp;취소(환불) 사유를 저장후 취소 처리하세요.<br>
		<textarea name="PaymentRemark" id="PaymentRemark" style="width:260px; height:60px"><?=$PaymentRemark?></textarea>
		<input type="button" name="Btn" id="Btn" value="메모저장" class="btn_inputSm01" style="width:80px; height:60px" onclick="PaymentRemarkSave(<?=$k?>,'<?=$LectureStart?>','<?=$LectureEnd?>','<?=$CompanyCode?>')" />
		<?
		}else{
		?>
		<textarea name="PaymentRemark" id="PaymentRemark" style="width:260px; height:60px"><?=$PaymentRemark?></textarea>
