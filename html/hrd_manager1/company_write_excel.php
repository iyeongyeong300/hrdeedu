...
<script src="https://spi.maps.daum.net/imap/map_js_init/postcode.v2.js"></script>
                ...
                <table width="100%" cellpadding="0" cellspacing="0" class="view_ty01">
                  ...
                  <tr>
                    <th>등록 샘플</th>
                    <td>
                		<input type="button" value="양식 다운로드" class="btn_inputLine01" onclick="location.href='./sample/사업주정보.xlsx'">&nbsp;&nbsp;&nbsp;&nbsp;
          				<input type="button" value="샘플 다운로드" class="btn_inputLine01" onclick="location.href='./sample/사업주정보(샘플).xlsx'">
					</td>
                  </tr>
                  <tr>
                    <th>파일 등록</th>
				...
				<div class="btnAreaTl02 pt50">
					<span class="fs16b fc333B"><img src="images/sub_title2.gif" align="absmiddle">업로드한 엑셀파일 데이터 확인</span>
				</div>
				<div class="tl pt15">
				...
				* 사업자 번호에 (중복Excel) 표기시 : 업로드한 엑셀파일에 중복되는 사업자번호가 존재합니다.(엑셀파일에 동일한 사업자번호 2건 이상)
				...
				<table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
				  ...
				  <tr>
					...
					$SQL = "SELECT *, 
								...
								(SELECT COUNT(idx) FROM CompanyExcelTemp WHERE CompanyCode=a.CompanyCode AND ID='$LoginAdminID') AS CompanyIDTempCount, 
								(SELECT COUNT(idx) FROM Company WHERE CompanyID=a.CompanyID) AS CompanyCodeCount, 
					...
					?>
				  <tr bgcolor="<?=$bgcolor?>">
					<td rowspan="2"><input type="checkbox" name="check_seq" id="check_seq" value="<?=$idx?>" style="width:17px; height:17px; background:none; border:none;"><br><img src="images/btn_edit04.gif" style="padding-top:5px; cursor:pointer" onclick="CompanyRegEdit('<?=$idx?>');"></td>
					<td rowspan="2"><?=$i?></td>
					<td><?=$CompanyName?></td>
					<td><?=$CompanyID2?></td>
					<td><?=$CompanyScale2?></td>
				...
				?>
				  <tr>
						<td height="50" class="tc" colspan="13">업로드한 엑셀파일이 없습니다.</td>
					</tr>
				<? } ?>
				</table>
				</form>
				...
				<tr>
					<td class="tl"><input type="button" id="BtnDelete" value="선택항목 삭제" class="btn_inputLine01" onclick="CheckedDelete()"></td>
					<td><input type="button" id="BtnSubmit" value="업로드 하기" class="btn_inputBlue01" onclick="CompleteSubmitOk()"></td>
					<td class="tr" ><input type="button" id="BtnList" value="목록" class="btn_inputLine01" onclick="location.href='<?=$PageName?>.php?pg=<?=$pg?>&sw=<?=urlencode($sw)?>&col=<?=$col?>'"></td>
				</tr>
			</table>
