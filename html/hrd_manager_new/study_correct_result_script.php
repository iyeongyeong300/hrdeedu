...
$MosaSelect = Replace_Check($MosaSelect); //모사의심
...
if(strpos($MosaSelect,'Y') !== false){
	$Mosa="Y";	
}else if(strpos($MosaSelect,'D') !== false){
	$Mosa="Y";	
}else{
	$Mosa="D";	
}
...
if($mode=="T" || $mode=="Y") {
	...
	//답안 테이블에 업데이트
	$Sql2 = "UPDATE TestAnswer SET ExamA_Point='$ExamA_Point', ExamB_Point='$ExamB_Point', ExamC_Point='$ExamC_Point', ScoreA=$ScoreA, ScoreB=$ScoreB, ScoreC=$ScoreC, TotalScore=$ExamTotalScore, TutorRemarkA='$TutorRemarkA', TutorRemarkB='$TutorRemarkB', TutorRemarkC='$TutorRemarkC' , Mosa='$MosaSelect' WHERE TestType='$TestType' AND Study_Seq=$Seq AND idx=$idx";
