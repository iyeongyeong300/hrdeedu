...
<table width="100%" cellpadding="0" cellspacing="0" class="list_ty01 gapT20">
	...
	<tr>
		...
		<td >
		<!-- Brad(2021.12.19) : 비환급과정을 위해 $StudyBeCount > 0 추가 -->
		<?if($StudyCount>0  || $StudyBeCount > 0) {?>
			<?if($ResultViewInputID) {?>
			처리자 : <?=$ResultViewInputID?><br />
			처리일 : <?=$ResultViewInputDate?><br />
			<button type="button" name="ResultViewBtn" id="ResultViewBtn" class="btn round btn_LBlue line" onclick="StudyEndComplete('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','ResultViewCancel')">확인 처리 취소</button>
			<?}else{?>
			<button type="button" name="ResultViewBtn" id="ResultViewBtn" class="btn round btn_LBlue line" onclick="StudyEndComplete('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','ResultView')">확인 처리</button>
			<?}?>
		<?}else{?>
		-
		<?}?>
		...
		<?if($StudyCount>0 || $StudyBeCount > 0) {?>
			...
			처리일 : <?=$StudyEndInputDate?><br />
			<button type="button" name="StudyEndBtn" id="StudyEndBtn" class="btn round btn_LBlue line" onclick="StudyEndComplete('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','StudyEndCancel')">마감 처리 취소</button>
			<?}else{?>
			<button type="button" name="StudyEndBtn" id="StudyEndBtn" class="btn round btn_LBlue line" onclick="StudyEndComplete('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','StudyEnd')">마감 처리</button>
			<?}?>
		<?}else{?>
		-
		<?}?>
		...
		<td >
			<p>
				<button type="button" name="CertBtn01" id="CertBtn01" class="btn round btn_LGray line" style="padding: 6px 10px 5px; width:121px;" onclick="StudyEndCertificatePrintPage('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','Y')">개인(환급)</button>
			</p>
			<p>
				<button type="button" name="CertBtn02" id="CertBtn02" class="btn round btn_LRed line" style="padding: 6px 10px 5px; width:121px;" onclick="StudyEndCertificatePrintPDF('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','Y')">환급 수료증 PDF</button> 
			</p>			
			<p>
				<button type="button" name="CertBtn03" id="CertBtn03" class="btn round btn_LGray line" style="padding: 6px 10px 5px; width:121px;" onclick="StudyEndCertificatePrintPage('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','N')">개인(비환급)</button>
			</p>
			<p>
				<button type="button" name="CertBtn04" id="CertBtn04" class="btn round btn_LRed line" style="padding: 6px 10px 5px; width:121px;" onclick="StudyEndCertificatePrintPDF('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','N')">비환급 수료증 PDF</button>
			</p>			
		</td>
		<td >
			<p>
				<button type="button" name="CertBtn05" id="CertBtn05" class="btn round btn_Green line" style="padding: 6px 10px 5px; width:121px;" onclick="location.href='study_end_result01.php?CompanyCode=<?=$CompanyCode?>&LectureStart=<?=$LectureStart?>&LectureEnd=<?=$LectureEnd?>'">개설현황</button>
			</p>
			<p>
				<button type="button" name="CertBtn06" id="CertBtn06" class="btn round btn_Green line" style="padding: 6px 10px 5px; width:121px;" onclick="location.href='study_end_result02.php?CompanyCode=<?=$CompanyCode?>&LectureStart=<?=$LectureStart?>&LectureEnd=<?=$LectureEnd?>'">신청현황</button>
			</p>
			<p>
				<button type="button" name="CertBtn09" id="CertBtn09" class="btn round btn_Green line" style="padding: 6px 10px 5px; width:121px;" onclick="location.href='receipt_confirm_excel.php?CompanyCode=<?=$CompanyCode?>&LectureStart=<?=$LectureStart?>&LectureEnd=<?=$LectureEnd?>'">수납 확인서</button>
			</p>
			<p>
				<button type="button" name="CertBtn07" id="CertBtn07" class="btn round btn_LGray line" style="padding: 6px 10px 5px; width:121px;" onclick="StudyEndDocument('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','Y')">환급과정만</button>
			</p>
			<p>
				<button type="button" name="CertBtn08" id="CertBtn08" class="btn round btn_LGray line" style="padding: 6px 10px 5px; width:121px;" onclick="StudyEndDocument('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>','N')">전체</button>
			</p>			
			<p>
				<button type="button" name="CertBtn09" id="CertBtn10" class="btn round btn_LGray line" style="padding: 6px 10px 5px; width:121px;" onclick="StudyEndDocumentPoll('<?=$CompanyCode?>','<?=$LectureStart?>','<?=$LectureEnd?>');">설문 결과</button>
			</p>
		</td>
	</tr>
	<?
