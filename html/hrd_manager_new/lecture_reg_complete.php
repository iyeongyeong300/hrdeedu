...
if($PackageYN=="Y") {
	...
	foreach($PackageLectureCode_Array as $PackageLectureCode_Array_value) {
		...
		//교육과정 정보를 불러오기
		$Sql_l = "SELECT LectureCode, Price, Price01, Price02, Price03,  Price01View, Price02View, Price03View,  TotalPassMid, TotalPassTest, TotalPassReport, ServiceType FROM Course WHERE LectureCode='$PackageLectureCode_Array_value'";
...
//echo "#1 CompanyScale =".$CompanyScale;
//echo "#1 Price01View =".$Price01View;
		//사업장 규모별 환급비용을 선정
		switch($CompanyScale) {
			case "A": //대규모 1000인 이상
				$rPrice = $Price03View;
...
if($error_count>0) {
	...
	$msg = "<font color='red'>오류</font>".$msg01.$msg02.$msg03;
