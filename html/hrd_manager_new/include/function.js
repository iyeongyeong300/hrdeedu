...
function StudyChangeStartEndPeriod() {
	...
	window.open(popupAddress, '수강기가변경', 'left=100, width=1400, height=900, menubar=no, status=no, titlebar=no, toolbar=no, scrollbars=yes, resizeable=no', 'batchPop');
}
...
function StudyEndCertificatePrintPDF(CompanyCode, LectureStart, LectureEnd, ServiceTypeYN) {
	if(ServiceTypeYN =="Y"){
		...
		var url = '/mypage/certificate_pdf03.php?CompanyCode=' + CompanyCode + '&LectureStart=' + LectureStart + '&LectureEnd=' + LectureEnd + '&ServiceTypeYN=' + ServiceTypeYN;
		window.open(url, 'certi', 'scrollbars=yes, resizable=yes, left=400, width=820, height=700');		
	}
}
