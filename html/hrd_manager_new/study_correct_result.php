...
if($Row) {
	...
	$MosaSelect_array = explode('|',$MosaSelect);
}else{
?>
<script type="text/javascript">
<!--
        ...
        <tr>                  
          <td style="padding:20px;">
			...
			<form name="Form1" method="POST" action="study_correct_result_script.php" target="ScriptFrame">
				...
				<input type="hidden" name="MosaSelect" id="MosaSelect" 		value="<?=$MosaSelect?>">
					...
					<ul class="list">
						<div class="comment" >
							...
							<select name="Mosa" id="Mosa" style="width:150px">
								<option value="N" <?if($MosaSelect[$k]=="N" || !$MosaSelect[$k]) {?>selected<?}?>>정상</option>
								<option value="D" <?if($MosaSelect[$k]=="D") {?>selected<?}?>>의심</option>
								<option value="Y" <?if($MosaSelect[$k]=="Y") {?>selected<?}?>>확정</option>
							</select>
						</div>
					</ul>
					<?
