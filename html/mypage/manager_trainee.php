...
?>
                ...
                <div class="contentArea" id="ContentGo">
                	...
                	<div class="titleZone">
                    	<h3>자사수강생 리스트</h3>
                        <!-- here -->
                        <div class="here"><a href="/">Home</a> > 온라인 학습실 > 교육담당자메뉴 > 자사수강생 리스트</div>
                        <!-- here // -->
                	</div>
                    <!-- page Title // -->
                        ...
                        <div class="PageZone">
                            ...
                            <div class="myManagerList">
                                ...
                                <div class="term">
									...
									<!-- btn -->
                                    <div class="btnSideArea"><span class="btnMyBase01"><a href="/mypage/manager_trainee_excel.php?LectureStart=<?=$LectureStart?>&LectureEnd=<?=$LectureEnd?>&CompanyCode=<?=$CompanyCode?>" target="_blank">개강전체 엑셀출력</a></span></div>
                                    <!-- btn // -->
                                </div>
								<?
                                // and LectureCode=a.LectureCode 23.04.20 추가
								$SQL2 = "SELECT DISTINCT
								a.CompanyCode, c.Category1, c.Category2, 
								(SELECT CategoryName FROM CourseCategory WHERE idx=c.Category1) AS Category1Name, 
								(SELECT COUNT(*) FROM Study WHERE LectureStart=a.LectureStart AND LectureEnd=a.LectureEnd AND CompanyCode=a.CompanyCode AND ServiceType=1 AND PassOk='Y' and LectureCode=a.LectureCode) AS StudyCount, 
								(SELECT COUNT(*) FROM Study WHERE LectureStart=a.LectureStart AND LectureEnd=a.LectureEnd AND CompanyCode=a.CompanyCode AND ServiceType=1 AND PassOk='N' and LectureCode=a.LectureCode) AS StudyBeCount, 
								(SELECT COUNT(*) FROM Study WHERE LectureStart=a.LectureStart AND LectureEnd=a.LectureEnd AND CompanyCode=a.CompanyCode AND ServiceType IN (3,5,9) AND PassOk='Y' and LectureCode=a.LectureCode) AS StudyCount2, 
								(SELECT COUNT(*) FROM Study WHERE LectureStart=a.LectureStart AND LectureEnd=a.LectureEnd AND CompanyCode=a.CompanyCode AND ServiceType IN (3,5,9) AND PassOk='N' and LectureCode=a.LectureCode) AS StudyBeCount2, 
                            	...
                            	<ul class="area">
                                    ...
                                    <li class="btnArea"><span class="btnDetail"><a href="Javascript:ManagerCourseCheck('<?=$LectureStart?>','<?=$LectureEnd?>','<?=$LectureCode?>');">상세보기</a></span></li>
                                </ul>
								<?
								$k++;
									}
