...
<?
	...
	$Sql = "SELECT * FROM PaymentSheet2 WHERE ID='$ID' AND (PayStatus='R' OR PayStatus='S' OR PayStatus='Y') AND idx=$pay_idx";
	...
	$Sql = "SELECT * FROM Study WHERE ID='$LoginMemberID' AND Seq=$Study_Seq";
	$Result = mysqli_query($connect, $Sql);
	$Row = mysqli_fetch_array($Result);
    ...
    $LGD_BUYER = $Name; //구매자명
    $LGD_PRODUCTINFO = "EK티처 한국어교사원격평생교육원 근로자훈련 수강료|".$PaymentSheet_idx."|".$LectureStart."|".$LectureEnd; //상품명
...
<div class="layerArea wid550">
	...
	<div class="infoArea">
		...
		<div class="comment_1">
			<ul>
				<li>교육생명, 교육과정, 교육 기간, 결제 금액을 확인 후 결제를 진행하여 주세요.</li>
			</ul>
		</div>
		<div class="info mt20">
