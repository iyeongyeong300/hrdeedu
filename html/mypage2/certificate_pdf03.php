...
if($NotPDF=="Y") {
	...
	while($ROW = mysqli_fetch_array($QUERY))
		...
		$pdf->SetFont('바탕','',19);
		$pdf->WriteHTML("                    위 사람은 (주)에이치알디이 솔류션");
		$pdf->Ln(10);
		$pdf->WriteHTML("                    근로자산업안전보건교육센터의 해당 ");
		$pdf->Ln(10);
		$pdf->WriteHTML("                    교육과정을 위와 같이 수료하였으므로 ");
		$pdf->Ln(10);
		$pdf->WriteHTML("                    이 증서를 수여합니다. ");
		...
		if($LectureCode=="AAAA1" || $LectureCode=="AAAA1") {
			...
			if($LectureCode=="AAAA1") {
				...
				$pdf->Image('../images/company_stamp2.png',128,247,20,21,'','' );
			}
			...
			//$pdf->Image('../images/company_stamp.png',148,238,20,21,'','');
                        $pdf->Image('../images/company_stamp2.png',138,238,20,21,'','');
		}
